...
import { getTodayInTimezone, getEntryAmountForPeriod, getActualAmountForPeriod } from '../utils/budgetCalculations';
...
const DashboardView = () => {
  ...
  const expenseDistributionData = useMemo(() => {
    ...
    const calculateOffBudgetTotalsForPeriod = (type, period) => {
      ...
      const budget = offBudgetEntries.reduce((sum, entry) => sum + getEntryAmountForMonth(entry, period.startDate, period.endDate), 0);
      const actual = offBudgetEntries.reduce((sum, entry) => sum + getActualAmountForPeriod(entry, actuals, period.startDate, period.endDate), 0);
    ...
    const calculateMainCategoryTotals = (entries, period) => {
        ...
        const actual = entries.reduce((sum, entry) => sum + getActualAmountForPeriod(entry, actuals, period.startDate, period.endDate), 0);
        return { budget, actual };
    };
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div className="lg:col-span-1 bg-white p-6 rounded-lg shadow-sm border flex flex-col">
          ...
          {overdueItems.length > 0 ? (
            <div className="space-y-3 overflow-y-auto custom-scrollbar h-[28rem]">
        ...
        {/* Right Column: Monthly Budget Summary */}
