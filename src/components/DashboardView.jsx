...
const DashboardView = () => {
  ...
  const { settings, categories, projects } = state;
  
  const [activeTab, setActiveTab] = useState('overview');

  const { actualTransactions, userCashAccounts, budgetEntries, isConsolidated, activeProject } = useTreasuryData();
  ...
  }, [userCashAccounts, actualTransactions]);
  ...
  const dashboardSparklineData = useMemo(() => {
    ...
    if (todayIndex < monthlyPeriods.length - 1) {
        ...
        for (let i = todayIndex + 1; i < monthlyPeriods.length; i++) {
            if (!monthlyPeriods[i]) continue;
            const period = monthlyPeriods[i];
            const netPlanned = budgetEntries.reduce((sum, e) => {
                const amount = getEntryAmountForMonth(e, period.startDate.getMonth(), period.startDate.getFullYear());
  ...
  }, [budgetEntries, actualTransactions, userCashAccounts, categories]);
  ...
  return (
    <div className="container mx-auto p-6 max-w-full space-y-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="bg-white p-6 rounded-lg shadow-sm border flex items-start gap-4"><div className="bg-green-100 p-3 rounded-full"><Wallet className="w-6 h-6 text-green-600" /></div><div><p className="text-sm text-gray-500">Trésorerie Actionnable</p><p className="text-2xl font-bold text-gray-800">{formatCurrency(totalActionableBalance, settings)}</p></div></div>
        <div className="bg-white p-6 rounded-lg shadow-sm border flex items-start gap-4"><div className="bg-red-100 p-3 rounded-full"><TrendingDown className="w-6 h-6 text-red-600" /></div><div><p className="text-sm text-gray-500">Dettes en Retard</p><p className="text-2xl font-bold text-red-600">{formatCurrency(totalOverduePayables, settings)}</p></div></div>
        <div className="bg-white p-6 rounded-lg shadow-sm border flex items-start gap-4"><div className="bg-yellow-100 p-3 rounded-full"><HandCoins className="w-6 h-6 text-yellow-600" /></div><div><p className="text-sm text-gray-500">Créances en Retard</p><p className="text-2xl font-bold text-yellow-600">{formatCurrency(totalOverdueReceivables, settings)}</p></div></div>
      </div>
          ...
          <motion.div key={activeTab} variants={tabContentVariants} initial="hidden" animate="visible" exit="exit">
            {activeTab === 'overview' && (
              <div className="bg-white p-6 rounded-lg shadow-sm border flex flex-col">
                <h2 className="text-xl font-semibold text-gray-800 mb-4 flex items-center gap-2 flex-shrink-0"><AlertTriangle className="w-5 h-5 text-yellow-500" />Actions Prioritaires <span className="text-gray-500 font-medium">- {activeProjectName}</span></h2>
                {overdueItems.length > 0 ? (
                  <div className="space-y-3 overflow-y-auto custom-scrollbar" style={{ height: '28rem' }}>
                    {overdueItems.map(item => { const project = isConsolidated ? projects.find(p => p.id === item.projectId) : null; return (
                          ...
                          <div className="flex justify-between items-center">
                              <div className="flex items-center gap-3">
                                  ...
                                  <div className="overflow-hidden">
                                      <p className="font-semibold truncate text-gray-800" title={item.thirdParty}>{item.thirdParty}{isConsolidated && project && <span className="text-xs font-normal text-gray-500 ml-1">({project.name})</span>}</p>
                                      <div className="text-xs text-gray-500 flex items-center gap-1.5"><span>{new Date(item.date).toLocaleDateString('fr-FR')}</span><span className="text-gray-500">({Math.floor((new Date() - new Date(item.date)) / (1000 * 60 * 60 * 24))}j en retard)</span></div>
                                  </div>
                              </div>
                ...
                ) : ( <div className="h-[28rem] flex items-center justify-center"><div className="text-center text-gray-500 py-10"><p>Aucune action prioritaire. Tout est à jour !</p></div></div> )}
              </div>
            )}
            {activeTab === 'budget' && <MonthlyBudgetSummary />}
            {activeTab === 'analysis' && (
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                <div className="bg-white p-6 rounded-lg shadow-sm border"><h3 className="text-lg font-semibold text-gray-800 mb-2 flex items-center gap-2"><LineChart className="w-5 h-5 text-blue-600" />Tendance de la Trésorerie (12 mois)</h3><SparklineChart data={dashboardSparklineData.balances} periods={dashboardSparklineData.periods} currencySettings={settings} showXAxis={true} xAxisLabels={dashboardSparklineData.periods.map(p => p.label.charAt(0))} /></div>
                <div className="bg-white p-6 rounded-lg shadow-sm border"><h3 className="text-lg font-semibold text-gray-800 mb-4 flex items-center gap-2"><PieChart className="w-5 h-5 text-purple-600" />{chartTitle}</h3>{expenseDistributionData.length > 0 ? (<ReactECharts option={getDonutChartOptions()} style={{ height: '250px' }} />) : (<div className="text-center text-gray-500 py-16"><p>Aucune sortie ce mois-ci.</p></div>)}</div>
              </div>
            )}
          </motion.div>
        </AnimatePresence>
