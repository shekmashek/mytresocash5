...
const SubscriptionPage = () => {
    ...
    const handleSubscribe = (plan) => {
        dispatch({ type: 'ADD_TOAST', payload: { message: `L'intégration du paiement pour le plan ${plan} est en cours de développement.`, type: 'info' } });
    };
    ...
    const plans = [
        { id: 'monthly', name: 'Mensuel', price: '12$', per: '/ mois', description: 'Flexibilité totale, sans engagement.', badge: null, buttonText: "S'abonner", buttonClass: 'bg-blue-100 text-blue-600 hover:bg-blue-200' },
        { id: 'annual', name: 'Annuel', price: '96$', per: '/ an', description: 'La meilleure offre pour un engagement à long terme.', badge: 'ÉCONOMISEZ 33%', buttonText: "S'abonner", buttonClass: 'bg-blue-600 text-white hover:bg-blue-700' },
        { id: 'lifetime', name: 'Accès à Vie', price: '499$', per: '/ à vie', description: 'Offre exclusive. Plus jamais à ce prix.', badge: 'Offre de Lancement', buttonText: 'Devenir un Visionnaire', buttonClass: 'bg-gradient-to-r from-amber-300 to-yellow-400 text-gray-900 hover:opacity-90' }
    ];
    ...
    return (
        <div className="container mx-auto p-6 max-w-6xl">
            ...
            <div className="flex flex-col lg:flex-row justify-center items-stretch gap-8">
                {plans.map(plan => {
                    const isCurrent = subscriptionStatus === plan.id;
                    return (
                        <div key={plan.id} className={`w-full max-w-sm p-8 bg-white rounded-2xl shadow-lg border flex flex-col ${isCurrent ? 'border-2 border-blue-600' : ''}`}>
                            ...
                            <div>
                                <button
                                    ...
                                    disabled={isCurrent}
                                    className={`w-full px-6 py-3 font-semibold rounded-lg transition-colors shadow-lg disabled:bg-gray-200 disabled:text-gray-500 disabled:cursor-not-allowed ${plan.buttonClass}`}
                                >
                                    {isCurrent ? 'Plan Actuel' : plan.buttonText}
                                </button>
